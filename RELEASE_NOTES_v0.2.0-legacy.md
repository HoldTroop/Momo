...
4. Install native messaging host:
   ...
   - Ensure the binary path points to `target/release/agent-bridge`
...
| `debugger` permission is optional | Requested at runtime via `chrome.permissions.request` on first CDP attach (`src/lib/permissions.ts`); denial degrades gracefully (CDP stays disabled) |
...
- **Native host binary**: `target/release/agent-bridge` (generated by `npm run build:bridge`)
