...
[![License](https://img.shields.io/badge/license-PolyForm%20Noncommercial%201.0.0-blue.svg)](https://github.com/HoldTroop/Momo/blob/main/LICENSE.md)
...
For comprehensive documentation, guides, and tutorials, see the [`docs/`](https://github.com/HoldTroop/Momo/blob/main/docs/README.md) directory.
...
- [Architecture](#architecture)
  ...
  - [Policy Engine Pipeline](#policy-engine-pipeline)
  - [Trust Boundary](#trust-boundary)
  - [MCP Tool-Call Sequence](#mcp-tool-call-sequence)
  - [Operating Modes](#operating-modes-side-by-side)
  - [Repo Map](#repo-map)
...
The diagrams below visualize that design across six angles: the full system topology, the policy pipeline that gates every action, the untrusted/trusted split that the whole design exists to enforce, the actual MCP request/response cycle, the two ways a task can be planned, and where each piece lives in the source tree. They're derived from the components, features, and config schema documented in this README — for the literal evaluation order and exact types, see `bridge/src/policy.rs` and `bridge/src/types.rs`.

### System Topology

```mermaid
flowchart TD
    subgraph UI["🖥️ Side Panel — React"]
        SP["Streaming chat • Task controls<br/>Confirm / Deny / Takeover modals<br/>Session management"]
    end

    subgraph EXT["🧩 Chrome Extension — Manifest V3 (TypeScript)"]
        direction LR
        SW["Service Worker<br/>AgentOrchestrator<br/>MessageRouter • CDP adapter<br/>WebSocket client"]
        CS["Content Scripts<br/>(ISOLATED world)<br/>Perception • AX-tree extraction<br/>DOM observation"]
        LIB["Shared Libraries<br/>Tool registry • Persistence (Dexie)<br/>Redaction engine • Selector heuristics"]
    end

    subgraph COMM["🔌 Communication"]
        direction LR
        WSC["WebSocket<br/>ws://127.0.0.1:9090-9100"]
        CDPP["Chrome DevTools Protocol<br/>trusted input"]
        MCPIO["MCP stdio<br/>NDJSON JSON-RPC 2.0"]
    end

    subgraph BRIDGE["🦀 Rust Bridge — agent-bridge (Tokio + Axum)"]
        direction LR
        WSS["WebSocket Server<br/>ConnectionManager"]
        POL["Policy Engine<br/>PolicyEngine::evaluate"]
        LLMG["LLM Gateway<br/>Anthropic Claude API / Ollama"]
        MCPD["MCP Tool Dispatch<br/>4 tools over stdio"]
    end

    subgraph STORE["💾 Storage"]
        direction LR
        IDB["IndexedDB (Dexie)<br/>Task state • WAL • checkpoints"]
        SQLDB["SQLite — ~/.momo/policy.db<br/>Policy config • Audit log"]
    end
    ...
    style UI fill:#8a5a1e,color:#fff,stroke:#c98a3a
    style EXT fill:#146b57,color:#fff,stroke:#1e9c7e
    style COMM fill:#3b3b91,color:#fff,stroke:#5b5bc4
    style BRIDGE fill:#8a3324,color:#fff,stroke:#c9502f
    style STORE fill:#4a4a4a,color:#fff,stroke:#777
```

*Every layer ultimately serves one goal: no action executes without bridge authorization.*

### Policy Engine Pipeline

```mermaid
flowchart TD
    A["Action Request<br/>(from LLM Gateway or an MCP tools/call)"] --> B{"Origin allowlist check<br/>subdomain-bounded matching"}
    B -- "origin not listed<br/>(empty list denies all)" --> DENY["🚫 Denied — fail closed<br/>written to audit log"]
    B -- "origin allowed" --> C{"Action in permitted_actions?<br/>click / type / navigate / scroll…"}
    C -- "not permitted" --> DENY
    C -- "permitted" --> D["Classify risk<br/>Low · Moderate · Sensitive"]
    D --> E{"Within token_budget_per_task?<br/>default 100,000"}
    E -- "budget exceeded" --> DENY
    E -- "within budget" --> F{"Risk ≥ confirmation_policy?"}
    F -- "yes" --> G["🖐️ Human-in-the-loop<br/>Confirm / Deny / Takeover modal"]
    G -- "user denies / takes over" --> DENY
    G -- "user confirms" --> H["✅ Execute via CDP trusted input"]
    F -- "no — below threshold" --> H
    H --> I["Write audit log entry<br/>origin, action, risk class, outcome"]

    style DENY fill:#7a2a22,color:#fff
    style H fill:#146b41,color:#fff
    style I fill:#146b41,color:#fff
    style G fill:#8a5a1e,color:#fff
```

*Any failed check short-circuits straight to denial — execution and the audit write only happen once every gate has passed.*

### Trust Boundary

```mermaid
flowchart LR
    subgraph UNTRUSTED["🔴 Untrusted Zone — Chrome Extension JS/TS"]
        AO["AgentOrchestrator<br/>(service worker)"]
        TE["Tool executors<br/>click, type, navigate, scroll"]
        PL["Perception layer<br/>(page-injected content scripts)"]
    end

    BOUNDARY{{"Trust boundary<br/>WebSocket + origin check<br/>separate OS process"}}

    subgraph TRUSTED["🟢 Trusted Zone — Rust Bridge (agent-bridge)"]
        PE["Policy Engine<br/>PolicyEngine::evaluate"]
        AL["Audit log (SQLite)<br/>the only source of 'allowed'"]
    end

    AO -->|"proposed action"| BOUNDARY
    TE -->|"proposed action"| BOUNDARY
    PL -->|"extracted content"| BOUNDARY
    BOUNDARY --> PE
    PE --> AL
    BOUNDARY -.->|"authorization decision"| AO

    style UNTRUSTED fill:#7a2a22,color:#fff
    style TRUSTED fill:#1f6e3d,color:#fff
    style BOUNDARY fill:#8a5a1e,color:#fff
```

*A bug anywhere in the untrusted JavaScript context cannot silently cross this boundary — the extension never self-authorizes, and only the Rust process can write "allowed" to the audit log.*
...
sequenceDiagram
    ...
    participant Bridge as Rust Bridge<br/>(agent-bridge --mcp)
    participant SW as Service Worker
    participant CS as Content Script

    Client->>Bridge: tools/call (e.g. execute_action)
    activate Bridge
    Bridge->>Bridge: PolicyEngine::evaluate
    alt denied by policy
        Bridge-->>Client: result.isError = true
    else approved
        Bridge->>SW: Command (over WebSocket)
        SW->>CS: request interactive elements
        CS-->>SW: pruned AX tree + stable el_XX refs
        SW->>SW: CDP trusted input (click / type / navigate)
        SW-->>Bridge: CommandResult
        Bridge->>Bridge: write audit log entry (SQLite)
        Bridge-->>Client: tools/call response
    end
    deactivate Bridge
```

*Stable `el_XX` refs from `get_interactive_elements` survive DOM mutation and are the only valid target for `execute_action` — raw CSS selectors are rejected.*

### Operating Modes, Side by Side

```mermaid
flowchart TB
    TASK(["User task"])

    subgraph MODEA["Mode A — Internal Orchestration (default)"]
        LLMG2["LLM Gateway<br/>Anthropic Claude API / Ollama"]
        PLANA["Orchestrator plans and executes<br/>the multi-step task itself"]
        LLMG2 --> PLANA
    end

    subgraph MODEB["Mode B — MCP over stdio"]
        EXTC["External MCP client<br/>(e.g. Claude Code)"]
        PLANB["Client plans the task,<br/>calls one tool at a time"]
        EXTC --> PLANB
    end

    TASK --> MODEA
    TASK --> MODEB
    PLANA --> POLICY2["Policy Engine<br/>(same gate, both paths)"]
    PLANB --> POLICY2
    POLICY2 --> EXEC["WebSocket → Service Worker → CDP execution"]

    style MODEA fill:#3b3b91,color:#fff
    style MODEB fill:#8a3324,color:#fff
    style POLICY2 fill:#8a5a1e,color:#fff
    style EXEC fill:#146b57,color:#fff
```

*Same policy engine gates both paths — only who plans the task differs.*
...
flowchart TD
    ROOT["Momo/"]
    ROOT --> SRC["src/ — Chrome extension (TypeScript)"]
    ROOT --> BRIDGEDIR["bridge/ — Rust backend (Tokio + Axum)"]
    ROOT --> TOOLSDIR["tools/ — dev & testing utilities"]
    ROOT --> DOCSDIR["docs/ — guides + ADRs"]
    ROOT --> ROOTFILES["manifest.json, vite.config.ts,<br/>package.json, tsconfig.json …"]

    SRC --> SWDIR["sw/ — service worker<br/>orchestrator.ts, message-router.ts,<br/>cdp-adapter.ts, ws-client.ts,<br/>alarm-manager.ts, bridge-port.ts"]
    SRC --> CONTENTDIR["content/ — content scripts<br/>(ISOLATED world)<br/>perception.ts, ax-extractor.ts,<br/>dom-observer.ts, human-input.ts"]
    SRC --> SPDIR["sidepanel/ — React UI<br/>index.tsx"]
    SRC --> LIBDIR["lib/ — shared libraries<br/>tool-registry.ts, persistence.ts,<br/>redaction.ts, selector.ts,<br/>task-queue.ts, permissions.ts"]

    BRIDGEDIR --> BSRC["src/<br/>main.rs, ws_server.rs, mcp_stdio.rs,<br/>mcp_tools.rs, policy.rs,<br/>llm.rs, types.rs"]

    DOCSDIR --> ADRDIR["adr/<br/>0001-policy-gate.md"]

    style ROOT fill:#146b41,color:#fff
    style SRC fill:#146b57,color:#fff
    style BRIDGEDIR fill:#8a3324,color:#fff
    style TOOLSDIR fill:#3b3b91,color:#fff
    style DOCSDIR fill:#4a4a4a,color:#fff
```

*See [Project Structure](#project-structure) below for the full annotated file tree.*
...
- Look for port conflicts: if another service is using ports 9090-9100, stop it or configure the bridge to use different ports
...
- Test with a permissive policy first, then tighten:
  ```json
  {
    "origin_allowlist": ["*"],
    "permitted_actions": ["click", "type", "navigate", "scroll"],
...
For persistent build issues, see [CONTRIBUTING.md](https://github.com/HoldTroop/Momo/blob/main/CONTRIBUTING.md) or [open an issue](https://github.com/HoldTroop/Momo/issues).
...
Contributions are welcome. Please read [CONTRIBUTING.md](https://github.com/HoldTroop/Momo/blob/main/CONTRIBUTING.md) and our [Code of Conduct](https://github.com/HoldTroop/Momo/blob/main/CODE_OF_CONDUCT.md) for detailed guidelines on commit conventions, pull requests, and code standards.

For common questions and troubleshooting, see our [FAQ](https://github.com/HoldTroop/Momo/blob/main/docs/FAQ.md).
...
This project is licensed under the [PolyForm Noncommercial License 1.0.0](https://github.com/HoldTroop/Momo/blob/main/LICENSE.md). Any noncommercial use is permitted; commercial use requires a separate license from the author.
